...
-- returns:
-- { affected_rows: 1 }
DROP FUNCTION IF EXISTS fetchq_reschedule(CHARACTER VARYING, INTEGER, TIMESTAMP WITH TIME ZONE);
CREATE OR REPLACE FUNCTION fetchq_reschedule (
	PAR_queue VARCHAR,
	PAR_docId INTEGER,
...
DROP FUNCTION IF EXISTS fetchq_reschedule(CHARACTER VARYING, INTEGER, TIMESTAMP WITH TIME ZONE, JSONB);
CREATE OR REPLACE FUNCTION fetchq_reschedule (
	PAR_queue VARCHAR,
	PAR_docId INTEGER,
	PAR_nextIteration TIMESTAMP WITH TIME ZONE,
